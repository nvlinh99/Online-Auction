  ...
  return (
    <ScrollMenu LeftArrow={LeftArrow} RightArrow={RightArrow}>
      {imgList &&
        imgList.map((img) => (
          <Card
            itemId={img.id} // NOTE: itemId is required for track items
            key={img.id}
            onClick={handleClick(img.id)}
            selected={isItemSelected(img.id)}
          >
            <img src={img.src} />
          </Card>
        ))}
      ...
      />
      <Card
        itemId={6} // NOTE: itemId is required for track items
        key={Math.random()}
